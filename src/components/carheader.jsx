...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (openRef.current) return; // Don't hide if mobile menu is open
      ...
      if (currentScrollY > lastScrollYRef.current && currentScrollY > threshold) {
        ...
        setIsVisible(false);
      } else if (currentScrollY < lastScrollYRef.current || currentScrollY <= threshold) {
  ...
  const navigationItems = [
    ...
    { label: "Testimonials", href: "/wwhyivish" },
    { label: "Blogs", href: "/bblog" },
  ];

  return (
    <motion.header
      className="fixed top-0 left-0 right-0 z-50 flex w-full justify-between items-center h-[78px] bg-transparent backdrop-blur-sm px-[82px] py-7 max-md:px-10 max-sm:px-5"
    ...
    >
      ...
      {/* Desktop Navigation */}
      <nav className="flex justify-center items-center gap-8 border bg-[rgba(255,255,255,0.08)] px-4 py-2 rounded-xl border-[#6B6B6B] max-md:gap-5 max-sm:hidden backdrop-blur-sm transition-all duration-300">
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetTrigger>
          <button className="hidden max-sm:block text-white hover:bg-white/10 rounded-lg p-2 transition-colors duration-200" onClick={() => setOpen(true)}>
        ...
        <SheetContent className="bg-black/95 backdrop-blur-sm text-white border-l border-[#6B6B6B] transition-transform duration-300">
          <div className="flex flex-col gap-6 mt-8">
            {navigationItems.map((item, index) => (
              <a
                key={index}
                href={item.href}
                className="text-[#D6D6D6] text-lg font-normal p-1 hover:text-white hover:translate-x-2 transition-all duration-200"
            ...
            <a
              href="/contact"
              className="text-black text-lg font-normal bg-white px-7 py-4 mt-4 text-center transition-all duration-200 hover:bg-gray-100 active:scale-95"
              onClick={() => setOpen(false)}
            >
              Contact Us
            </a>
